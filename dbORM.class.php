...
class dbORM {
	...
	private $suffix ="cls_";//CHANGE YOUR SUFFIX
	...
	*/
	...
	{
		$class = new ClassMember($field, $name, null, null);
	...
	private function _writeConstructor()
	{
		...
		$out = "\t\t\$SQL = \"select * from `".$this->_tableName."` where `".$this->_keyField->getField()."` = '\".mysql_real_escape_string(\$id).\"' limit 1\";\n";
		$out.= "\t\t\$result = mysql_query(\$SQL) or die(\"Error in SQL Syntax: \$SQL,\" . mysql_error());\n\n";
		$out.= "\t\tif (\$row = mysql_fetch_array(\$result)) {\n";
	...
	{	
		...
		$out.= "\t\t\$result = mysql_query(\$SQL) or die(\"Error in SQL Syntax: \$SQL,\" . mysql_error());\n\n";
		$out.="\t\t\$items = array();\$i=0;\n";
		$out.= "\t\twhile (\$row = mysql_fetch_array(\$result)) {\n";
	...
	private function _writeUnique()
	{
		...
		if(count($arr)>0)
		{
			...
			foreach ($arr as $IndexField)$out.= "and `".$IndexField->getField()."`='\".\$".$IndexField->getField().".\"' ";
	...
	private function _writeIndexes()
	{
		...
		foreach ($this->_indexFields as $IndexField)
		{
			...
			if($IndexField->_index=='INDEX')
			{
				...
				$out.= "\t\t\$SQL = \"select * from `".$this->_tableName."` where ".$IndexField->getField()."='\".\$id.\"'\";\n";
				$out.= "\t\t\$result = mysql_query(\$SQL) or die(\"Error in SQL Syntax: \$SQL,\" . mysql_error());\n\n";
				$out.="\t\t\$items = array();\$i=0;\n";
				$out.= "\t\twhile (\$row = mysql_fetch_array(\$result)) {\n";
	 ...
	 private function _writeInsert()
	{
		...
		foreach ($this->_dbFields as $field)
		{
			if ($field->getField() != $this->_keyField->getField())
			{
				if(!(stripos($field->getField(),'timestamp')===false)) continue;
				$out.="`".$field->getField()."`,";
				$values.="\t\t'\".mysql_real_escape_string(\$this->".$field->getName().").\"'\n,";
			}
		}
		$out=substr($out,0,strlen($out)-1).") values(\n".substr($values,0,strlen($values)-2).")\";\n";
		//$out.="where `".$this->_keyField->getField()."`='\".mysql_real_escape_string(\$this->_".$this->_keyField->getName().").\"' limit 1\";\n";
		$out.= "\t\t\$result = mysql_query(\$SQL);\n";
		$out.= "\t\tif (\$result === false)\n";
		$out.= "\t\t\treturn null;\n";
		$out.= "\t\telse{\$this->".$this->_keyField->getName()."=mysql_insert_id();\n";
		$out.= "\t\t\treturn \$this;}\n";
		$out.= "\t}\n\n";		
		return $out;
	}
	private function _writeUpdate()
	{
		...
		foreach ($this->_dbFields as $field)
		{
			...
			{
				if(!(stripos($field->getField(),'timestamp')===false)) continue;
				$out.="\t\t`".$field->getField()."`='\".mysql_real_escape_string(\$this->".$field->getName().").\"'\n,";
			}
		}
		$out=substr($out,0,strlen($out)-1);
		$out.="\t\twhere `".$this->_keyField->getField()."`='\".mysql_real_escape_string(\$this->".$this->_keyField->getName().").\"' limit 1\";\n";
		$out.= "\t\t\$result = mysql_query(\$SQL);\n";
		$out.= "\t\tif (\$result === false)\n";
		$out.= "\t\t\treturn null;\n";
		$out.= "\t\telse\n";
	...
	private function _writeDelete()
	{
		...
		$out = "\tpublic function Delete() {\n";
		$out.= "\t\t\$SQL = \"Delete from `".$this->_tableName."` where `".$this->_keyField->getField()."`='\".mysql_real_escape_string(\$this->".$this->_keyField->getName().").\"' limit 1\";\n";
