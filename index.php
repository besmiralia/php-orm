<?php
	...
	{
		$db=$_GET['db'];
		mysqli_select_db($link, $db) or die("Cannot connect to database ".$db);
	}
...
<html>
	...
	<body>
		<table><tr><td><ul>
			<?php
				$res =$link->query("select SCHEMA_NAME from information_schema.SCHEMATA");
				$class='';
		...
		<form method='post' action=''>
			<select name='tables[]' multiple='multiple' size='20'>
				<?php
					$res=$link->query("select * from information_schema.TABLES T where `TABLE_SCHEMA`='".$db."' AND `TABLE_TYPE`='BASE TABLE'") or die(mysqli_error($link));
					while($r=mysqli_fetch_array($res))
					{
				...
				if($_POST){
					...
					*/
					...
					foreach($_POST["tables"] as $k=>$v){
						...
						$class = new dbORM($v,$v);
						//write columns
						$ress=$link->query("SELECT * FROM information_schema.COLUMNS where TABLE_SCHEMA='".$db."' AND TABLE_NAME='".$v."'") or die(mysqli_error($link));
						...
						//write primary key
						$ress=$link->query("SELECT COLUMN_NAME FROM information_schema.`KEY_COLUMN_USAGE` K INNER JOIN information_schema.TABLE_CONSTRAINTS T ON K.CONSTRAINT_NAME=T.CONSTRAINT_NAME AND K.TABLE_NAME=T.TABLE_NAME AND K.TABLE_SCHEMA=T.TABLE_SCHEMA	where K.TABLE_SCHEMA='".$db."' AND K.TABLE_NAME='".$v."' AND CONSTRAINT_TYPE='PRIMARY KEY' order by K.ORDINAL_POSITION") or die(mysqli_error($link));
						while($rr=mysqli_fetch_array($ress))
						{
						...
						echo "<br/>";
						//write indexes
						$ress=$link->query("SHOW INDEX FROM ".$v." WHERE Non_Unique=1") or die(mysqli_error($link));
						while($rr=mysqli_fetch_array($ress))
						{
							/*
